...
先頭に 4Byte の単一データがあり、以降に 3Byte のデータが隙間なく並んでいます。
...
構造体内に固定サイズ配列を含める手段として、固定サイズバッファ（fixed T[]）が用意されています。

これを使えば良いのですが、制約が多く扱い難い印象です…

- 配列型 に 組み込み型（byte, short, int, ...）しか指定できない（3Byteの配列を作れない）
- フィールドを（構造体全体を） readonly にできない
- unsafe必須
...
固定サイズの配列を分解して1行ずつべたで書けば、固定サイズバッファ を使用せずに同じデータ構造を定義できますが、想像するだけでげんなりします。（宣言だけでなくgetterも必要ですし…）
...
1. ファイルを追加します。
   ...
   VisualStudioプロジェクト → 追加 → 新しい項目 → テキストテンプレート(.tt) 

2.  テンプレートを書きます。★でコメントを埋めています。
...
#>
    ...
    {
        public readonly int Param;
        <#= CreateDeclarationFields(0, count) #>	//★作成したい固定配列の数だけフィールドを定義する
...
テンプレートコード内で、ローカル関数 / 文字列補間($"") を使うと、VSにシンタックスエラーを指摘されちゃいました。（コードは生成されますが、エラーが取れないので気持ち悪い…）
...
構造体 に 固定サイズの配列 を実装するため、T4 を使ってフィールドをべた書きしてみました。

構造体で固定サイズ配列を扱うよりスマートな実装 や、固定サイズバッファ（fixed）の safe な使用法 がありましたらコメント下さい。
